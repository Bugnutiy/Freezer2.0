...
private:
    ...
    uint32_t _min_change_time, _tmr_min_change_time;
    void _setState(bool state, bool now = 0);

public:
    Relay(uint8_t pin, bool truth = 1, uint32_t min_change_time = 0);
    ~Relay();
    bool set(bool state);
    ...
    bool getNextState();
    void change();
    void setMinChangeTime(uint32_t t);
...
Relay::Relay(uint8_t pin, bool truth, uint32_t min_change_time)
{
    pinMode(pin, OUTPUT);
    _pin = pin;
    _truth = truth;
 ...
 */
void Relay::setMinChangeTime(uint32_t min_change_time)
{
    _min_change_time = min_change_time;
}
 ...
 * @brief
 *
 * @param now true для мгновенного включения
 */
void Relay::resetTimer(bool now = false)
{
    if (now)
        _tmr_min_change_time = millis() - _min_change_time;
    else
 ...
 *
 * This function sets the relay's state to true and updates the relay's pin accordingly.
 *
 * @return true if the relay's state was changed, false otherwise.
 */
bool Relay::on()
{
    return set(1);
 ...
 *
 * This function sets the relay's state to false and updates the relay's pin accordingly.
 *
 * @return true if the relay's state was changed, false otherwise.
 */
bool Relay::off()
